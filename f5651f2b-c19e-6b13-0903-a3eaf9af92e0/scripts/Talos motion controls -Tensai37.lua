...
local LEFT_POS_X = -102.735
local LEFT_POS_Y = 111.126
local LEFT_POS_Z = -0.822

local LEFT_ROT_X = -0.354
local LEFT_ROT_Y = -0.731
local LEFT_ROT_Z = 1.040
...
local function get_mc_state(component)
    if component == nil then
        return nil
    end

    return UEVR_UObjectHook.get_or_add_motion_controller_state(component)
end

local function bind_hand(component, hand_index)
    local st = get_mc_state(component)
    if st == nil then
        return false
    end
...
local function apply_left_offsets()
    local st = get_mc_state(left_mc_pivot)
    if st == nil then
        return
    end

    local pos = vec(LEFT_POS_X, LEFT_POS_Y, LEFT_POS_Z)
    local rot = vec(LEFT_ROT_X, LEFT_ROT_Y, LEFT_ROT_Z)

    pcall(function() st:set_location_offset(pos) end)
    pcall(function() st:set_position_offset(pos) end)
    pcall(function() st:set_rotation_offset(rot) end)

    pcall(function() st.location_offset = pos end)
    pcall(function() st.position_offset = pos end)
    pcall(function() st.rotation_offset = rot end)
end
