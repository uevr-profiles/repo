...
local MenuJustOpened=false
 ...
 function OperateTactiNav() 
 
	if TactiNavVisible then
		if rThumb then	
			TactiNav:InpAxisKeyEvt_MouseWheelAxis_K2Node_InputAxisKeyEvent_0(-1)
		elseif lThumb then
			TactiNav:InpAxisKeyEvt_MouseWheelAxis_K2Node_InputAxisKeyEvent_0(1)
		end
		if math.abs(ThumbRX)>20000 then
			TactiNav:InpAxisEvt_Turn_K2Node_InputAxisEvent_0(ThumbRX/32767)
		end
		if math.abs(ThumbRY)>20000 then
			TactiNav:InpAxisEvt_LookUp_K2Node_InputAxisEvent_1(-ThumbRY/32767)
		end
...
function SetMouseLocation(player)
	...
	if TactiNav==nil or not UEVR_UObjectHook.exists(TactiNav) then
		TactiNav=UEVR_UObjectHook.get_first_object_by_class(TactiNav_C,false)
		--TactiNav.Widget.AttachChildren[1]:SetCollisionEnabled(0)
		--TactiNav.Widget.AttachChildren[1]:SetGenerateOverlapEvents(false)
		--TactiNav:EnableInput(player)
	else
	...
	if Cursor~=nil and (isMenu or not TactiNavVisible) then
	...
	--and right_hand_component~=nil  then
		...
		MouseY = MouseY + ((rotDiff.Rotation.Y)^1.00000000000000005/math.pi*180  /45 * 1080)
		MouseX = MouseX + ((rotDiff.Rotation.Z)^1.00000000000000005/math.pi*180 /45*1920   )
