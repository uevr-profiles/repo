	...
	--if pawn.Children~=nil then 
		...
		for i , comp in ipairs(MeshArray) do
			if comp["ShowcaseWeapon?"] ~=nil  then
				...
				if comp.BP_WeaponComponent ~=nil and comp.BP_WeaponComponent.Mag~=nil then
			...
			elseif string.find(comp:get_full_name(),"TactiNav") then
				local OvMats3= comp.SkeletalMesh.OverrideMaterials
				--print(#OvMats)
				if OvMats3 ~= nil then
					for k , comp2 in ipairs(OvMats3) do
						if comp2.ScalarParameterValues ~= nil then
							--print(comp2parent)--comp2.ScalarParameterValues[1].ParameterValue)
						end
						comp2:SetScalarParameterValue(MatScalString,0.0)
				...
				--	
					for s , comp1 in ipairs(ChildrenOfWeapon) do
						...
						if comp1.MaterialInstance ~=nil then
							local OvMatsC= comp1.MaterialInstance
							--for d , comp2 in ipairs(OvMatsC) do
								OvMatsC:SetScalarParameterValue(MatScalString,0.0)
							--end
						end
