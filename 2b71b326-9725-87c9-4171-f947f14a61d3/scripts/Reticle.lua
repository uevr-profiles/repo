...
local sightTexture_name = "Texture2D /Game/ArtOriginal/textures/Architecture/T_nocturnal01.T_nocturnal01"
...
local function init_static_objects()
	...
	--sightTexture_name = "Texture2D /Engine/StarterContent/textures/T_ground_Moss_D.T_ground_Moss_D"
    ...
    if not reusable_hit_result then return false end
	 ...
	 reusable_hit_result5 = StructObject.new(hitresult_c)
    if not reusable_hit_result5 then return false end
...
local function attach_components_to_weapon(weapon_mesh)
	...
	--	end
		if reticle_plane_component then
			scope_mesh = right_hand_component--get_scope_mesh(weapon_mesh)
			if scope_mesh == nil then
				print("Failed to find scope mesh")
				return
			end
			...
			reticle_plane_component:K2_SetRelativeRotation(temp_vec3:set(0, 80, 90), false, reusable_hit_result, false)
...
--    end
--    return false
if reusable_hit_result5.Distance< 300 then
	return true
else return false end

end

local function switch_scope_state()
   current_scope_state = is_scope_active()
   --
...
local function Recalculate_FOV(c_pawn)	
	if Get_ScopeHmdDistance()>=5.5 then
		--pcall(function()
		fov= 30*(Get_CurrentScopeFOV(c_pawn)* (2* math.atan(5/Get_ScopeHmdDistance())/(90/180*math.pi)))/94	
		--end)
	else 
	--pcall(function()
