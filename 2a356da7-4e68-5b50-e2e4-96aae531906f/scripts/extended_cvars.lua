...
local r_Atmosphere = get_cvar_int("r.Atmosphere"); local r_Atmosphere_s = false 
local r_FilmGrain = get_cvar_int("r.FilmGrain"); local r_FilmGrain_s = false 
local r_Materials = get_cvar_int("r.PostProcessing.DisableMaterials"); local r_Materials_s = false 
...
local function write_config()
    ...
    if r_Materials_s then config = config .. string.format("r_Materials=%d\n", r_Materials) end
...
local function read_config()
    ...
    if config_data then
        for key, value in config_data:gmatch("([^=]+)=([^\n]+)\n?") do
            ...
            elseif key == "r_Materials" then r_Materials = num_val; r_Materials_s = true
...
local function apply_cvars()
    ...
    set_cvar_int("r.Atmosphere", r_Atmosphere)
    set_cvar_int("r.FilmGrain", r_FilmGrain)
    set_cvar_int("r.PostProcessing.DisableMaterials", r_Materials)
...
uevr.lua.add_script_panel("Extended CVARs", function()
    ...
    if imgui.collapsing_header("Original CVARs") then
        ...
        changed, new_value = add_imgui_combo("r.Atmosphere", r_Atmosphere, {"Disabled", "Enabled"})
        if changed then r_Atmosphere = new_value; needs_config_write = true; r_Atmosphere_s = true end
        ...
        changed, new_value = add_imgui_combo("r.PostProcessing.DisableMaterials", r_Materials, {"Enabled", "Disabled"})
        if changed then r_Materials = new_value; needs_config_write = true; r_Materials_s = true end
